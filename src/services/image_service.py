...
class ImageTool:
    ...
    async def reset_name_image(self, filename: str) -> str:
        # Siempre guardamos con extensión .webp
    ...
    async def save_image(self, image_file: UploadFile) -> str | None:
        ...
        try:
            ...
            filename = await self.reset_name_image(image_file.filename)
            file_location = os.path.join(self.path_image, filename)
            cv2.imwrite(file_location, image_resized, [cv2.IMWRITE_WEBP_QUALITY, 85])

            logging.info(f"Imagen guardada como WebP: {filename}")
            return filename
    ...
    async def get_image(self, file_name: str):
        try:
            ...
            # Siempre es WebP
            return FileResponse(file_path, media_type="image/webp", filename=file_name, status_code=status.HTTP_200_OK)
        except Exception as e:
